...
smtp_obj.ehlo()
smtp_obj.starttls()
smtp_obj.login("hogadashboard@gmail.com", "rarv jjoq frpr tbxh")
...
def userlogin():
    ...
    with db.begin() as conn:
        ...
        for info in login:
            print("HOLA:", info)
            if check_password_hash(info.password,password) and (info.user_activated == 1):
            ...
            elif check_password_hash(info.password,password) and (info.user_activated == 0):
                return {"message": "user needs to activate account"}
        return {"message": "user inputted wrong email or password"}
