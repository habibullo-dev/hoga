...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const otherWidgets = Array.from(widgets).filter(
  ...
  function createWidget(widgetId) {
    ...
    if (!existingWidget) {
      const widgetHtml = `${widgetId}.html`; // Path to HTML content
      const widgetCss = `${widgetId}.css`; // Path to CSS styling
      const widgetScript = `${widgetId}.js`; // Path to JS
  ...
  loadWidgetStylesheet("island.css"); // Replace with your actual CSS file path
